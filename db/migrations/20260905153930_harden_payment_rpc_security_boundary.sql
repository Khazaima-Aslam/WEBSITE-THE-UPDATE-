...
create or replace function private.customer_payment_overview_impl()
returns table(
  quote_id uuid,
  reference text,
...
$$;
revoke all on function private.customer_payment_overview_impl() from public,anon;
grant execute on function private.customer_payment_overview_impl() to authenticated;
...
as $$ select * from private.customer_payment_overview_impl(); $$;
...
create or replace function private.customer_submit_payment_impl(
  p_quote_id uuid,
...
begin
  ...
  if v_quote.status not in ('confirmed'::public.quote_status,'delivered'::public.quote_status) then raise exception 'Payment proof can only be submitted after the quotation is confirmed.' using errcode='55000'; end if;
  if p_method not in ('bank_transfer'::public.payment_pref,'jazzcash'::public.payment_pref,'easypaisa'::public.payment_pref) then raise exception 'Payment proof is only supported for bank transfer, JazzCash or EasyPaisa.' using errcode='22023'; end if;
...
$$;
revoke all on function private.customer_submit_payment_impl(uuid,public.payment_pref,numeric,text,text) from public,anon;
grant execute on function private.customer_submit_payment_impl(uuid,public.payment_pref,numeric,text,text) to authenticated;
...
security invoker
set search_path=private,public,pg_temp
as $$ select private.customer_submit_payment_impl($1,$2,$3,$4,$5); $$;
...
create or replace function private.staff_quote_payment_overview_impl(p_quote_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
$$;
revoke all on function private.staff_quote_payment_overview_impl(uuid) from public,anon;
grant execute on function private.staff_quote_payment_overview_impl(uuid) to authenticated;
...
security invoker
set search_path=private,public,pg_temp
as $$ select private.staff_quote_payment_overview_impl($1); $$;
...
create or replace function private.staff_review_payment_impl(
  p_payment_id uuid,
  p_decision text,
...
$$;
revoke all on function private.staff_review_payment_impl(uuid,text,text) from public,anon;
grant execute on function private.staff_review_payment_impl(uuid,text,text) to authenticated;
...
security invoker
set search_path=private,public,pg_temp
as $$ select * from private.staff_review_payment_impl($1,$2,$3); $$;
