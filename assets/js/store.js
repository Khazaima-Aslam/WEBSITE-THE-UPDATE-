/* ═══════════════════════════════════════════════════════════════
   ...
   Single production data-access layer for public pages and admin.
   ═══════════════════════════════════════════════════════════════ */
(function (global) {
  "use strict";
  ...
  function specsText(specs) {
    ...
    return Object.entries(specs)
      .filter(([k]) => !["grade", "size", "badge", "quality", "source_catalogue_id"].includes(k))
      .map(([k, v]) => `${k}: ${v}`)
      .join("; ");
  }

  function normalise(p) {
    const images = arr(p.images).length ? arr(p.images).slice() : (p.img ? [p.img] : []);
    return {
      id: p.id,
  ...
  function fromCatalogue(row) {
    ...
    const gallery = arr(row.images).map((x) => x && x.url).filter(Boolean);
    const main = row.main_image || gallery[0] || "";
    const images = [main, ...gallery.filter((u) => u && u !== main)].filter(Boolean);
    return normalise({
      id: row.id,
      sku: row.sku,
      title: row.name,
  ...
  const LocalStore = {
    ...
    products: {
      ...
      async save(product) {
        ...
        try { localStorage.setItem(DRAFT_KEY, JSON.stringify(list)); } catch (_) {}
        return p;
      },
      async remove(id) {
      ...
      async replaceAll(list) { try { localStorage.setItem(DRAFT_KEY, JSON.stringify(list.map(normalise))); } catch (_) {} },
      async discardDraft() { try { localStorage.removeItem(DRAFT_KEY); } catch (_) {} },
      hasDraft() { try { return !!localStorage.getItem(DRAFT_KEY); } catch (_) { return false; } }
    },
    categories: {
      async list() {
        const groups = typeof GROUPS !== "undefined" ? GROUPS : {};
        return Object.entries(groups).map(([slug, d], i) => ({ slug, name: d.label, order: i, children: arr(d.categories) }));
      }
    },
    files: {
      async upload(file) {
        return { path: `local://${safeFileName(file.name)}`, url: URL.createObjectURL(file), size: file.size, name: file.name, type: file.type, persisted: false };
      },
      async remove() {}
    },
    projects: {
      async create(p) {
        const row = { id: randomId(), reference: `CKA-P-${Date.now()}`, persisted: false, ...p };
        let list = [];
        try { list = JSON.parse(localStorage.getItem(PROJECT_KEY)) || []; } catch (_) {}
        list.unshift(row);
        try { localStorage.setItem(PROJECT_KEY, JSON.stringify(list)); } catch (_) {}
        return row;
      },
      async list() { try { return JSON.parse(localStorage.getItem(PROJECT_KEY)) || []; } catch (_) { return []; } }
    },
    inquiries: { async create(i) { return { ...i, persisted: false }; } }
  };

  function createSupabaseStore(client, projectBucket) {
    const B = projectBucket || "project-uploads";

    async function categoryIdFor(p) {
      ...
      const { data, error } = await client.from("categories").select("id,name").eq("name", name).limit(2);
    ...
    return {
      ...
      products: {
        async list() {
          const { data, error } = await client.from("v_catalogue").select("*").order("display_order", { ascending: true });
          if (error) throw error;
          const products = arr(data).map(fromCatalogue);
          const { data: cats, error: catErr } = await client.from("categories").select("id,name").eq("is_active", true);
          if (!catErr && cats) {
            const map = new Map(cats.map((c) => [c.name, c.id]));
            products.forEach((p) => { p.category_id = map.get(p.category) || null; });
          }
          return products;
        },
        async save(product) {
          ...
          const { data, error } = await client.from("products").upsert(payload).select("id").single();
          if (error) throw error;
          const { data: row, error: rowErr } = await client.from("v_catalogue").select("*").eq("id", data.id).single();
          if (rowErr) throw rowErr;
          const saved = fromCatalogue(row);
        ...
        async replaceAll() { throw new Error("Full catalogue replacement is disabled on production. Use reviewed merge imports."); },
        async discardDraft() {},
      ...
      categories: {
        async list() {
          ...
          if (error) throw error;
          const rows = arr(data);
          return rows.filter((r) => !r.parent_id).map((p, i) => ({
            id: p.id, slug: p.slug, name: p.name, order: p.display_order == null ? i : p.display_order,
            children: rows.filter((c) => c.parent_id === p.id)
              .sort((a, b) => (a.display_order || 0) - (b.display_order || 0)).map((c) => c.name)
          }));
        }
      },
      files: {
        async upload(file, meta) {
          ...
          if (error) throw error;
          return { path, url: null, size: file.size, name: file.name, type: file.type || "application/octet-stream", persisted: true, bucket: B };
        },
        async remove(path) {
          if (!path) return [];
      ...
      projects: {
        async create(p) {
          const scope = [p.material ? `Requirement: ${p.material}` : null, p.qty ? `Quantity: ${p.qty}` : null, p.message || null]
            .filter(Boolean).join("\n");
          const file = p.file && p.file.persisted ? p.file : null;
          const { data: reference, error } = await client.rpc("submit_project_with_file", {
            p_client_name: String(p.name || "").trim(), p_phone: String(p.phone || "").trim(),
            p_email: p.email || null, p_company: p.company || null, p_project_name: p.material || null,
            p_project_type: p.ptype || null, p_location: p.city || null, p_budget_min: p.budgetMin ?? null,
            p_budget_max: p.budgetMax ?? null, p_expected_completion: p.expectedCompletion || null,
            p_scope: scope || null, p_file_path: file?.path || null, p_file_name: file?.name || null,
            p_file_mime: file?.type || null, p_file_size: file?.size || null
          });
          if (error) throw error;
      ...
      inquiries: {
        async create(i) {
          const { data, error } = await client.rpc("submit_inquiry", {
            p_name: String(i.name || "").trim(), p_message: String(i.message || "").trim(),
            p_email: i.email || null, p_phone: i.phone || null, p_subject: i.topic || null,
  ...
  if (global.supabase && typeof global.supabase.createClient === "function") {
    sb = global.supabase.createClient(global.CKA_CONFIG.supabaseUrl, global.CKA_CONFIG.supabaseAnonKey);
  ...
  if (sb && global.document && global.document.body && !global.document.body.classList.contains("adm")) {
    ...
    script.src = "assets/js/public-forms.js?v=27";
    script.async = true;
    script.onerror = () => console.warn("CKA PUBLIC: public form handlers failed to load");
