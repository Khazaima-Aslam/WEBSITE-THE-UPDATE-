...
(function (global) {
  ...
  async function enhance(force){
    ...
    const existing=document.getElementById("ops-payment-verification");
    if(existing&&!force)return;
    loading=true;
    try{
      const data=await rpc("staff_quote_payment_overview",{p_quote_id:currentQuoteId});
      if(existing)existing.remove();
      const section=document.createElement("section");section.className="ops__section";section.id="ops-payment-verification";
  ...
  async function review(id,decision){
    const input=Array.from(document.querySelectorAll("[data-payment-note]")).find((el)=>el.dataset.paymentNote===id);const notes=input?.value?.trim()||null;
    ...
    try{await rpc("staff_review_payment",{p_payment_id:id,p_decision:decision,p_review_notes:notes});toast(`Payment ${decision}.`);await enhance(true);}catch(err){toast(err.message||"Could not review payment.",true);}
  }

  document.addEventListener("click",(event)=>{
    const open=event.target.closest("[data-open-quote]");if(open){currentQuoteId=open.dataset.openQuote;document.getElementById("ops-payment-verification")?.remove();setTimeout(()=>enhance(false),120);setTimeout(()=>enhance(false),420);}
  ...
  const body=document.getElementById("ops-drawer-body");if(body)new MutationObserver(()=>{if(currentQuoteId&&!document.getElementById("ops-payment-verification"))setTimeout(()=>enhance(false),40);}).observe(body,{childList:true,subtree:true});
})(window);
